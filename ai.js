...
sendBtn.addEventListener("click", async () => {
  ...
  // Tawagin ang TensorFlow.js AI prediction function
  const aiAnswer = await runTfModel(userText);

  if (aiAnswer) {
    setTimeout(() => {
      addChatBubble("bot", aiAnswer);
    }, 800);
  } else {
    setTimeout(() => {
...
async function loadAndPredict(inputText) {
  // Halimbawa: load pre-trained text classification model (mock example)
  const modelUrl = 'https://your-model-host.com/model.json'; // palitan ng actual model URL
  const model = await tf.loadLayersModel(modelUrl);

  // Preprocess inputText (depende sa model)
  const processedInput = preprocess(inputText);

  // Gamitin ang model para mag-predict
  const prediction = model.predict(processedInput);

  // I-process ang prediction result para sagot
  const answer = postprocess(prediction);

  return answer;
}

// Placeholder functions para sa pre/post processing
function preprocess(text) {
  // Convert text to tensor, tokenize, etc. (depende sa model)
  // Simpleng example lang ito:
  return tf.tensor([1, 2, 3, 4]); 
}

function postprocess(prediction) {
  // I-convert ang prediction tensor pabalik sa readable text o label
  return "This is a mock AI answer.";
}
